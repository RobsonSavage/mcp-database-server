...
| File | Purpose |
|---|---|
| `robsonsavage-database-server-3.0.5.tgz` | npm tarball built via `npm pack` — installs the `rs-database-server` global binary |
...
```bash
npm install -g ./robsonsavage-database-server-3.0.5.tgz
```
